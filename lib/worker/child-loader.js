...
function handleCall(requestData) {
    const workerReceivedTime = process.hrtime();
    const request = new Request(requestData);
    const response = Response.from(request);

    response.transportTime = {
        mainSendTime: requestData.transportTime.mainSendTime,
        workerReceivedTime: workerReceivedTime
    };
    ...
    return new Promise(resolve => {
        ...
        .then(result => {
            ...
            response.transportTime.workerSendTime = process.hrtime();
